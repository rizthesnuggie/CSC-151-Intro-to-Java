/*Modify the class named Billing that includes three overloaded computeBill() methods for a photo book store.
...
public class Billing {
    ...
    public static double computeBill(double amt, int quantity) {
        return((amt*quantity)+((amt*quantity)*0.08));
    }
    public static double computeBill(double amt, int quantity, double coupon) {
        return(((amt*quantity)-coupon)+((amt*quantity)-coupon)*0.08);
    }
}
